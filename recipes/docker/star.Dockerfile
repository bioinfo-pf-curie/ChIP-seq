...
# real path from baseDir: /home/nservant/Apps/geniac/build/workDir/recipes/conda/star.yml
ADD star.yml /opt/star.yml

RUN yum install -y which   \
&& yum clean all \
&& conda env create -f /opt/star.yml \
&& echo "source activate star_2.7.6a" > ~/.bashrc \
&& conda clean -a


ENV PATH /usr/local/envs/star_2.7.6a/bin:$PATH
